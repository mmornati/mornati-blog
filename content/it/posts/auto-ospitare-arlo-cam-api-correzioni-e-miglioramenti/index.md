...
- Open Source
- Home Assistant
description: 'Come auto-ospitare l''emulatore di stazione base Arlo arlo-cam-api: stack Docker con RTSP on-demand tramite MediaMTX, sidecar Flask arlo-snapshot personalizzato, e le tre patch che ho contribuito upstream per correggere un bug di avvio Flask, aggiungere un beacon keepalive, e abilitare snapshot-on-motion.'
cover: cover.jpg
showHero: true
---
...
- **Nessun abbonamento.** Le telecamere sono VMC4040P (Arlo Pro 4), il che significa che continuano a funzionare senza abbonamento — ma gli alert di movimento sono limitati ai soli push, si perde la timeline cloud, e il giorno in cui Arlo deciderà di ritirare il livello gratuito, le telecamere diventeranno costosi fermacarte. Il local-only elimina quel rischio.
...
- **Nessun rischio di disattivazione remota.** Arlo può, a livello cloud, deprecare un modello o bloccare un seriale. Se avete mai avuto una stampante che si è briccata perché HP ha deciso che era obsoleta, capite.
...
Il lavoro iniziale di reverse-engineering è [Meatballs1/arlo-cam-api](https://github.com/Meatballs1/arlo-cam-api). Non è stato attivamente mantenuto per un po'. Il fork attivamente mantenuto è [brianschrameck/arlo-cam-api](https://github.com/brianschrameck/arlo-cam-api), che pubblica un'immagine Docker (`bschrameck/arlo-cam-api:latest`) che uso come base per tutto in questo post. Tutte le patch di produzione che descrivo sotto sono state contribuite come PR a quel fork.
...
Notate le tre righe di bind-mount su `arlo-cam-api` che caricano `./server.py`, `./api/api.py`, e `./config.yaml` sopra le copie dell'immagine upstream. Questo è il workaround delle patch di produzione che mi permette di rilasciare i fix delle PR #29, #30, e #31 senza dover forkare l'immagine. Tornerò su questo nella sezione "Patch di produzione" più sotto.
...
```
Trigger PIR
  -> la telecamera invia pirMotionAlert su TCP/4000
    -> l'handler server.py si attiva
      -> POST http://arlo-snapshot:8000/snapshot/<serial>
        -> arlo-snapshot chiama POST /device/<serial>/userstreamactive {active:true}
        -> dorme STREAM_WARMUP_SEC (6s) per far aprire il server RTSP
        -> apre rtsp://<cam-ip>:555 o 554 con PyAV, decodifica un frame
        -> ridimensiona a MAX_WIDTH=1280, codifica JPEG quality 75
        -> memorizza in un dict in memoria indicizzato per serial
HA still_image_url GET
  -> http://<server>:8000/snapshot/<serial>
    -> restituisce il JPEG memorizzato (nessun RTSP, nessuna interazione con la telecamera)
```
...
- **`maxReaders: 5`** — Home Assistant apre multiple connessioni RTSP per telecamera (preview + flusso principale + rilevatore Frigate se capita di usarlo). Il default di MediaMTX che è 2 è troppo basso e si ottengono errori `maximum reader count reached`. Cinque è comodo per HA + uno o due viewer esterni.
...
Un nuovo `BeaconThread` che, ogni `BeaconIntervalSeconds` (default 60s), invia una leggera `statusRequest` a ogni telecamera vista tramite traffico in ingresso. La `statusRequest` restituisce un ACK + una risposta di stato, che fa doppio lavoro come probe di liveness.
...
L'intervallo di 60 secondi è conservativo. La soglia per `MaxMissedBeaconTime` è 30, ma la cadenza esatta con cui Arlo conta i beacon persi non è documentata. 60s vi dà un margine di sicurezza di 30x che sembrava comodo; potreste scendere fino a 10s se voleste, al costo di un carico leggermente più alto sulla stazione base.
...
Il default è `false` così che i deployment senza `arlo-snapshot` che gira non siano influenzati. Sul mio deployment di produzione è `true`, e i trigger PIR risultano in un JPEG `<serial>` fresco che arriva nello store in memoria di `arlo-snapshot` in ~10 secondi.
...
Il workaround è il bind-mount. Il container `arlo-cam-api` espone `/opt/arlo-cam-api/` con i file sorgente. Tre righe in `docker-compose.yaml` sovrappongono le mie copie patchate sopra l'immagine upstream:
...
server/
  ...
  config.yaml         # patchato (BeaconIntervalSeconds, SnapshotOnMotion, soppressione webhook)
  mediamtx.yml
  arlo-snapshot/
    Dockerfile
    app.py
...
I file `.patch` sono vere uscite `git format-patch` contro l'HEAD upstream `brianschrameck/arlo-cam-api` al momento in cui ho iniziato il lavoro. Per rigenerarli:
