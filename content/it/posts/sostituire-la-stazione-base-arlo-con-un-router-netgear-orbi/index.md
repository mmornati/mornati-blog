...
Tre anni fa mi sono trasferito in una casa dove il proprietario precedente aveva lasciato una singola stazione base Arlo nello studio al piano di sopra e tre telecamere Pro 4 sparse nel giardino. L'app Arlo funzionava, il cloud era gratuito durante il periodo di prova, e il sistema mi mandava fedelmente gli snapshot quando il postino passava davanti al cancello. Poi la prova è scaduta, la batteria della *Camera Giardino 2* è morta in quattro giorni, e la telecamera nell'angolo del giardino rifiutava di connettersi più di una volta su due — il suo segnale WiFi arrivava dalla stazione dello studio attraverso due muri di mattoni e una persiana metallica. Suona familiare?
...
L'Orbi RBR760 è un sistema WiFi mesh di qualità enterprise, non un adattatore USB consumer. Esso:

- Supporta 802.11ax (WiFi 6) con corretta negoziazione power-save
- Ha le capability ShortPreamble, STBC, RIFS e AMPDU corrette
...
La rete guest dell'Orbi è speciale: ha un suo bridge (`br-guest`), un suo sottoinsieme (`192.168.2.0/24` di default), un suo server DHCP (`dni_guest_udhcpd`, non dnsmasq), e una sua zona firewall con `forward=REJECT`. Tutti questi vincoli esistono per ragioni di sicurezza nel firmware residenziale, e li affronteremo uno per uno nelle prossime sezioni.
...
Verificate con `iptables -L FORWARD -n -v --line-numbers` — le regole utente devono essere alle righe 1-3, e `ODM_FORWARD_TOP` deve apparire alla riga 4 o successiva. Se l'ordine è invertito, la telecamera non si registrerà.
...
Lo script di avvio completo, idempotente, che gestisce le particolarità Netgear è a [`rbr760/S99arlo`](https://github.com/mmornati/arlo-base-station/blob/main/rbr760/S99arlo) nel repository di accompagnamento. Sono 60 righe, commenti inclusi. I tre bit non ovvi sono estratti sotto.
...
Se volete testare l'endpoint-to-end prima di toccare le telecamere, fate questo dal server:
...
Se le regole mancano dopo un reboot ma `S99arlo` è in `/etc/rc.d/`, allora sta gareggiando con l'init Netgear. Aumentate il numero di avvio (es. `S99arlo` → `S991arlo`).
...
# Forse un firmware Netgear è stato auto-pushato (avete dimenticato di disabilitare gli aggiornamenti?)
nvram show | grep auto_
...
- **Post 3** — Integrazione Home Assistant tramite sensori REST, entità Generic Camera (still + stream), la dashboard Lovelace usando `camera_view: auto`, Tailscale per l'accesso remoto, e Scrypted se volete HomeKit.
