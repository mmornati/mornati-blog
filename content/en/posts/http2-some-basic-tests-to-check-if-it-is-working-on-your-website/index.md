...
tags:
  ...
  - web
description: >-
  Basic HTTP/2 tests to verify your website and server support the protocol
  using curl, online tools, and nginx configuration checks.
---

I didn't know before, but today I discovered that all my Docker containers were already HTTP/2 enabled.

## Overview

To test if your server or website supports HTTP/2, use this [online service](https://tools.keycdn.com/http2-test). It will tell you whether your website supports HTTP/2 or not.

## Online Test

![Image for post](/images/http2-some-basic-tests-to-check-if-it-is-working-on-your-website/00-0_Q3aVt-pKzNG0Zn_b.png)

![Image for post](/images/http2-some-basic-tests-to-check-if-it-is-working-on-your-website/01-0_Q3aVt-pKzNG0Zn_b.png)

I then tried to investigate a little more, hoping to understand why it was working (I know: a normal person would have said "Cool, nothing to do", but... you know... **I'm an engineer** :)).

## Curl Test

Using curl (you need a recent version with HTTP/2 support — on macOS you can install it with _brew_) you are able to force a request using the HTTP/2 protocol and check the verbose output.
...
The main thing you need for HTTP/2 is to enable it on your web server.
To check with nginx, run the `nginx -V` command.
...
Check then if HTTP/2 is enabled in the nginx server configuration file.

![Image for post](/images/http2-some-basic-tests-to-check-if-it-is-working-on-your-website/02-0_CcA3Nrk4SoHNMPZJ.png)

![Image for post](/images/http2-some-basic-tests-to-check-if-it-is-working-on-your-website/03-0_CcA3Nrk4SoHNMPZJ.png)

## Tools

There are already plenty of tools online to check, use, and configure the HTTP/2 protocol.
The only thing we have to know is that we need to code the website in a completely different way to allow better performance with HTTP/2. For example: we used to rely on JavaScript and CSS joiners and minifiers. If we keep plenty of small (minified) files without merging them, we will get better performance on our websites.
There are some good resources about HTTP/2 online:
...
I made load and benchmark tests using an HTTP/2 tool (which should only use the HTTP/2 protocol).

![Image for post](/images/http2-some-basic-tests-to-check-if-it-is-working-on-your-website/04-0_LQsO_ewxwUBKfuej.png)

![Image for post](/images/http2-some-basic-tests-to-check-if-it-is-working-on-your-website/05-0_LQsO_ewxwUBKfuej.png)
...
![Image for post](/images/http2-some-basic-tests-to-check-if-it-is-working-on-your-website/07-0_-Y55O5AX31J-jY3e.png)

I'm not sure the data are really comparable... the HTTP/2 test took less time with better performance. But, for what I know, it could simply be the tools used for the test.

Anyway, let's start using HTTP/2 and prepare your servers and applications for it.

_Originally published at_ [https://blog.mornati.net](https://blog.mornati.net/http2-some-basic-tests-to-check-if-it-is-working-on-your-website/) _on October 10, 2016._