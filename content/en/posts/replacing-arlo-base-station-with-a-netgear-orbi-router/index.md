...
Verify with `iptables -L FORWARD -n -v --line-numbers` — the user rules must be at lines 1-3. On this firmware the ODM chains may not be referenced at all in a freshly booted `FORWARD` chain (they get wired up only after certain UCI/guest-zone events); what matters is that your rules are the first ACCEPTs in the chain. If the order is reversed, the camera will not register.
...
The complete, idempotent, Netgear-quirk-handling startup script is at [`rbr760/S99arlo`](https://github.com/mmornati/arlo-base-station/blob/main/rbr760/S99arlo) in the companion repo. It is 92 lines, including comments. The excerpt below is a simplified version for illustration; the three non-obvious bits are highlighted.
