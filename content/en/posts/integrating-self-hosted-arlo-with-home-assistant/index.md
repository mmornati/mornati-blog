...
I kept `aarlo` installed on this deployment. For the past three years, the four `sensor.aarlo_battery_level_*` entities were the ones whose values triggered the mobile push notifications on my phone. During the migration, when I rebooted the server, or the cameras went offline, or I was testing a new PR, those push notifications were the early-warning signal that something was wrong. But there is a catch I had to be honest about: `aarlo` is a cloud client. The battery values it reported came from the Arlo cloud API — `pyaarlo` authenticates against `myapi.arlo.com` and reads device state from the cloud's device list, not from the cameras or the local emulator. The moment the cameras stop being registered with the Arlo cloud — which is the whole point of this series — `aarlo` has nothing left to poll.
...
Three lines. `backend: sse` only selects the event-stream transport `pyaarlo` uses for its cloud connection (SSE instead of the MQTT stream); it does not create a session-free mode and it does not point `aarlo` at the local emulator. Everything `aarlo` exposes still comes from the Arlo cloud. Once the cameras are no longer registered there, the cloud-facing entities have nothing to report — the camera, motion, sound, siren and snapshot entities go `unavailable`, and the four battery sensors we keep around only freeze at the last value the cloud reported. They are a historical record, not a live feed.
...
Arming mirrors how the real Arlo ecosystem works, but it is worth being precise about what it actually changes. The PIR element itself is a **passive IR detector that is physically always on in both states** and draws negligible power. Arming only changes the *response policy*: when motion fires, an **armed** camera wakes its SoC and radio, streams ~10 s of video (`PIRAction: Stream`, `DefaultMotionStreamTimeLimit: 10`), then goes back to sleep after sending a `motionTimeoutAlert`; a **disarmed** camera simply ignores the event. That 10 s stream on every motion event is what eats battery — which is why disarming is the single biggest battery lever you have, and why the camera can sleep freely between events either way. Two details matter. First, the cameras boot **`Armed` by default** thanks to the `DefaultPIRTargetState` config option in `arlo-cam-api` (values `Armed` | `Disarmed`, default `Armed`), so a freshly booted camera behaves like it did out of the box. Second, the arm/disarm toggle is exactly the control the battery chapter below depends on: disarming a camera you don't need right now stops that motion-triggered streaming cold.
...
How non-trivial? I measured it: with the keepalive beacon at 60 seconds, the cameras armed, and the dashboard polling snapshots, the fleet lost roughly **3% of battery per hour** overnight — a full charge gone in a day and a half. The [battery drain chapter](#measured-battery-drain--the-real-numbers) below shows every measurement. That is why the wake dial exists, and why the whole `arlo_wake` machinery above was built the way it was.
...
The `arlo_armed` master plus the per-camera `camera_<cam>_armed` gating means a PIR event only costs battery when you actually care about that camera right now. Disarm the camera (or the whole fleet) and the PIR wakes simply stop firing — the same behaviour as the real Arlo system when you set a mode.
...
## Measured Battery Drain — The Real Numbers

Everything above is designed around one constraint: the camera battery. When I first moved the fleet to the self-hosted stack, the defaults were close to "always warm", and the numbers I measured overnight were a wake-up call. Here is the full data, so you can see exactly what each knob costs.

### The overnight baseline: 60 s beacon, armed, HA wake, dashboard open

The night of the test (22:41 → 09:30, 10.8 h) ran with the keepalive beacon at 60 s, all cameras **armed**, the HA periodic wake active (15 min), and the Lovelace dashboard open — the configuration most people would start with:

| Camera | Before | After | Δ | Rate |
|---|---|---|---|---|
| Jardin 1 | 42% | 1% | **−41%** | **−3.8%/h** |
| Jardin 2 | 47% | 17% | −30% | −2.8%/h |
| Entrée | 80% | 46% | −34% | −3.1%/h |
| Portail | 59% | 100% | +41% | charging |

All three battery cameras lost roughly a third of their charge in a single night. That is a full charge gone in about a day and a half — untenable for a solar-powered fleet.

But look at the connectivity columns: `CameraOffline` stayed frozen at 37 s, 15 s, and 34 s all night. **The beacon did its job — no camera dropped off WiFi.** So the overnight drain was *not* a connectivity problem. As we will see, it was dominated by the cameras being armed with active views: motion-triggered streaming, the HA polling storm, and the HA periodic wake — not the beacon.

### The four drain sources

I broke the overnight number down into four separate drains:

1. **Motion-triggered video streaming while armed.** Every PIR event on an armed camera wakes the SoC and streams ~10 s of video. In the overnight window the log shows **116 `pirMotionAlert` events total** (Jardin 1 = 30, Jardin 2 = 16, Entrée = 12, Portail = 0), and *every single one* is followed ~10 s later by a `motionTimeoutAlert` with `StreamDuration: 10`. This is the smoking gun: the battery cost is a function of **motion frequency in each camera's field of view**, not of the beacon. Jardin 1 — the camera with the busiest view — drained at **~5.8%/h** in the later armed test. The beacon's own CPU wakes (~1–2%/h at 60 s) are real but secondary; a TCP `statusRequest` is not a real base-station WiFi beacon (the real base station keeps the camera's CPU asleep in 802.11 PS-Poll power-save and only wakes it for an actual event), yet the dominant cost is clearly the streaming.
2. **The HA snapshot polling storm.** `arlo-snapshot` logged **986 `GET /snapshot/` requests in 12 h**, peaking at **658 requests/hour between 18:00 and 20:00**. The culprit was the picture-glance dashboard cards: HA keeps polling the `camera_proxy` / `image_url` of every card that has ever been on screen — even after the dashboard is closed. Every poll is a wake (if asleep) plus a JPEG grab.
3. **The HA periodic wake (`userstreamactive`).** With `duration: 1800`, the stream window never closed, so the cameras never slept between the 15-minute ticks. The `UserStreamed` counter exploded: Jardin 1 **536/h**, Jardin 2 399/h, Entrée 326/h, Portail 1443/h. Compare Entrée's ~5 streams/h baseline → 392/h measured = a **77× amplification**. The wake was firing far more often than intended.
4. **Snapshot on every wake.** Each wake-then-snapshot cycle opens an RTSP session for ~30 s and grabs a JPEG. Cheap on its own; brutal when multiplied by the three above.

### The 200 s experiment: the deep-sleep trap

I then pushed the beacon interval to its extreme: `BeaconIntervalSeconds: 200`, with the cameras `Disarmed`.

The result over 3 h: **0% drain** on every camera. But there is no free lunch — the cameras were not conserving energy, they were **asleep**. With a 200 s interval the cameras stopped hearing from a base station often enough and entered deep sleep: the radio is effectively off, port 4000 is closed, and nothing can reach it. The beacon fired on schedule and got "no response" every cycle. A sleeping camera can only be woken by its own firmware cycle (2–5 h) or by physically pressing the sync button.

Here is the part we initially got wrong: **the deep sleep was caused by the 200 s interval, not by disarming.** The earlier 60 s test was *also* `Disarmed` — and those cameras stayed connected the whole night. Disarming only stops the motion-response streaming; it does not make a camera unreachable. Only an interval that long does that.

This exposed a second problem: after a container restart, `known_devices` starts empty, so the beacon has *nothing* to probe until each camera happens to re-register on its own. That is exactly the bug the DeviceDB seeding in [Post 2](/self-hosting-arlo-cam-api-patches-and-improvements/)'s PR #30 fix addresses.

The "great battery" result is real — but only because the cameras are unreachable.

### The ARMED + 100 s test

Finally I re-armed the fleet and set the beacon to 100 s — a sane interval — to measure what arming actually costs. The result splits cleanly along motion activity, not the armed flag:

- **Portail, armed, zero motion events**: **94% → 94% over 2 h 45 min** — **0% drain** while answering every 100 s beacon. Armed with an empty field of view is essentially free.
- **Jardin 1, armed, 30 motion events in the same window**: **100% → 84% (−16%)** ≈ **~5.8%/h** — a 2 h 45 min burst from the camera that sees the most activity.

So the earlier "Portail 100% → 94% ≈ 2%/h" reading was actually from the *60 s, disarmed* run — not the armed test. Corrected: **drain tracks motion events per camera, and arming was never a connectivity fix.**

### The honest verdict

This stack is not at Arlo level. The real base station *is* the WiFi AP, so it can keep cameras in a low-power active state on its own network; a TCP beacon every 100 s is a coarser approximation of that. And unlike the real cloud service, every motion event on an armed camera costs a ~10 s stream of local video. That is the honest trade-off of self-hosting: you trade cloud convenience for full ownership, and you manage battery with configuration instead of Arlo's black-box optimization.

But with the configuration knobs above you can get a viable solar-powered fleet:

- **`DefaultPIRTargetState`** — the *response-policy* switch, not a power switch. `Disarmed` = PIR fires, camera ignores it (no streaming, ~0% drain, fully reachable as long as the beacon interval stays sane); `Armed` = motion triggers a ~10 s stream (the real cost — a busy view can hit ~5.8%/h). Arm what you need, disarm what you don't.
- **`BeaconIntervalSeconds`** — the *reachability* dial: keep it at 60–100 s. At 200 s cameras deep-sleep and become unreachable for hours; at 60–100 s they stay attached at a negligible cost. This is not a battery dial — motion activity is.
- **`arlo_wake_mode`** — the HA-side dial from the previous section (`off` / `periodic` / `on-demand`).
- **`scan_interval: 300`** on the REST sensors — battery percentage does not change in five-minute windows.
- **Keep the dashboard closed** — the picture-glance cards poll `image_url` continuously; that 658 GETs/h storm is the most insidious drain because it is invisible in HA's UI.

> **The key insight: measure motion events per camera, not battery % alone.** If a camera drains but shows few or no `pirMotionAlert` events, look elsewhere — a too-sparse beacon (re-registration cycles), snapshot polling storms, or HA polling keeping `UserStreamActive=1`. If it drains *and* has many motion alerts, the drain is the armed motion-streaming — disarm it or reduce its sensitivity.
...
- **Wake Mode selector at the top of the Cameras panel.** `off` (no auto-wake, cameras sleep until you trigger them), `periodic` (every 15 minutes, the default), `on-demand` (wake only when the Lovelace card is being viewed). On vacation, keep the cameras armed (PIR stays active) and leave the mode on `on-demand` or `periodic`; disarm what you don't need to cut the wake cost. On a normal day, leave it on `periodic`.
- **PIR event triggers an immediate wake + snapshot.** The `arlo_wake_on_pir_<cam>` automations fire whenever the camera's PIR counter increments — but only when the camera (and the fleet) is armed. Mode is `off`, or the camera is disarmed → no wake. Otherwise → wake, then 6 s later a fresh JPEG is in the sidecar's in-memory store. The Lovelace card picks up the new image on the next refresh tick.
...
- **`aarlo` is a cloud client; the REST sensors are the real battery source.** `pyaarlo` reads battery and device state from the Arlo cloud API, not from the cameras or the local emulator. Once the cameras leave the cloud, the `sensor.aarlo_battery_level_*` entities freeze at the last value the cloud reported and eventually go `unavailable` — there is no local path for them to refresh from. The REST sensors in `sensor.arlo_<cam>_status` read the same `BatPercent` attribute directly from the local emulator's `/device/<serial>` endpoint, so they keep working with no cloud dependency.
