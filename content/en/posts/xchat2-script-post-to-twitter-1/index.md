...
- Create a new application in your twitter account (going to [OAuth](https://developer.twitter.com/) page). All key and secrets must be substituted where you initialize your twitter Api (twitter.Api line on the scripts). One thing to remember is the application you are creating must have **read and write** access to your account.
- Change the target username (in the script is **twitter_nick**). For example, if you want to receive a private message to your twitter account when a DirectMessage is sent to you in xchat, here you have to put your twitter username.
- Install script in your xchat (usually $HOME/.xchat2 folder). You can also test the script just by loading it using the menu option _Load plugin or script_.

## Testing

That's all. If all worked well, you can test sending yourself a private message on IRC and you should receive a private message on twitter :D Let me know if you have any better idea to do the same thing, or if you encounter any issues or have fixes on the proposed script.