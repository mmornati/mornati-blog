...
- **No subscription.** The cameras are VMC4040P (Arlo Pro 4), which means they keep working without a subscription — but motion alerts are throttled to push only, you lose the cloud timeline, and the day Arlo decides to retire the free tier, the cameras turn into expensive paperweights. Local-only removes that risk.
...
Note the three bind-mount lines on `arlo-cam-api` that load `./server.py`, `./api/api.py`, and `./config.yaml` over the upstream image's copies. This is the production-patch workaround that lets me ship the fixes from PRs #29, #30, and #31 without forking the image. I will come back to this in the "Production patches" section below.
...
```
PIR trigger
  -> camera sends pirMotionAlert over TCP/4000
    -> server.py handler fires
      -> POST http://arlo-snapshot:8000/snapshot/<serial>
        -> arlo-snapshot calls POST /device/<serial>/userstreamactive {active:true}
        -> sleeps STREAM_WARMUP_SEC (6s) for the RTSP server to open
        -> opens rtsp://<cam-ip>:555 or 554 with PyAV, decodes one frame
        -> resizes to MAX_WIDTH=1280, encodes JPEG quality 75
        -> stores in an in-memory dict keyed by serial
HA still_image_url GET
  -> http://<server>:8000/snapshot/<serial>
    -> returns the stored JPEG (no RTSP, no camera interaction)
```
...
The 60-second interval is conservative. The threshold for `MaxMissedBeaconTime` is 30, but the exact cadence at which Arlo counts missed beacons is not documented. 60s gives you a 30x safety margin which felt comfortable; you could go as low as 10s if you wanted, at the cost of slightly more basestation load.
...
The workaround is bind-mounting. The `arlo-cam-api` container exposes `/opt/arlo-cam-api/` with the source files. Three lines in `docker-compose.yaml` overlay my patched copies over the upstream image:
...
server/
  ...
  config.yaml         # patched (BeaconIntervalSeconds, SnapshotOnMotion, webhook suppression)
  mediamtx.yml
  arlo-snapshot/
    Dockerfile
    app.py
