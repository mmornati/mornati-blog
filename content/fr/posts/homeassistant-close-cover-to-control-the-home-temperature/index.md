---
title: 'HomeAssistant : Fermer les volets pour contrôler la température domestique'
tags:
- automation
- weather
- home-assistant
...
Comme pour le script de simulation de présence, j'utilise un déclencheur `time_pattern` car je souhaite vérifier constamment les conditions pendant une période donnée.  
...
Voici les conditions pour s'assurer que nous fermons au bon moment.
...
**Temps**  
Les volets que je souhaite contrôler sont orientés vers le sud, donc je n'exécute l'automatisation que l'après-midi lorsque le soleil fait directement face aux fenêtres : `after: "12:30:00" before: "18:00:00"`
...
Cette partie — qui je sais semble complexe — vérifie si l'automatisation a déjà été déclenchée (jusqu'à cette exécution) **ou** n'a jamais été exécutée (nécessaire pour la première exécution ou lorsque la dernière exécution est si ancienne que les données historiques ont été supprimées).  
Nous utilisons la propriété `last_triggered`, en vérifiant si elle est `none` ou si la dernière exécution date de plus de **8 heures**. Pourquoi 8 heures ? Il suffit d'empêcher une ré-exécution dans la même plage horaire (12h30-18h00) tout en permettant son exécution le jour suivant (18h00-12h30). 8 couvre les deux : plus de 6,5 heures (18h00-12h30) et moins de 18,5 heures (12h30-18h00).
...
La deuxième condition vérifie la température dans la pièce où se trouvent les volets — elle doit être **supérieure** à 20°C.  
...
Tous les volets que je souhaite contrôler sont réglés à 40%. Pas complètement fermés, mais suffisamment pour réduire la lumière du soleil entrant dans la pièce.  
J'ai ajouté un autre volet d'une autre pièce sans créer d'action séparée — l'exposition est la même.

Si nous associons le tout, le script est le suivant :
...
En regardant notre automatisation, le vrai déclencheur est la température : l'extérieur est supérieur de 2 degrés à l'intérieur et l'intérieur est supérieur à 20 degrés.  
...
Nous pourrions ajouter un deuxième déclencheur pour la température intérieure, mais les déclencheurs utilisent la logique **OR** : si l'un d'eux est vrai, l'action s'exécute (bien que les conditions puissent l'empêcher).
