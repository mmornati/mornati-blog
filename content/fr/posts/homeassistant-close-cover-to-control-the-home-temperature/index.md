...
Aujourd'hui, je vais vous présenter un script simple pour améliorer l'efficacité énergétique de votre maison en régulant la température intérieure en fonction des valeurs extérieures.  
Ceci est une première version simple basée sur un seul point de température. J'en ai une nouvelle version prête à tester, mais je dois attendre des jours plus chauds 😅
...
Comme pour le script de simulation de présence, j'utilise un déclencheur `time_pattern` car je veux vérifier constamment les conditions pendant une période spécifique.  
...
Voici les conditions pour s'assurer que la fermeture se fait au bon moment.
...
Cette partie — je sais qu'elle semble complexe — vérifie si l'automatisation a déjà été déclenchée (jusqu'à cette exécution) **ou** n'a jamais été exécutée (nécessaire pour la première exécution ou lorsque la dernière exécution était si ancienne que les données historiques ont été supprimées).  
Nous utilisons la propriété `last_triggered`, en vérifiant si elle est `none` ou si la dernière exécution date de plus de **8 heures**. Pourquoi 8 heures ? Il suffit d'empêcher une ré-exécution dans la même plage horaire (12h30-18h00) tout en permettant son exécution le lendemain (18h00-12h30). 8 heures couvre les deux : plus de 6,5 heures (18h00-12h30) et moins de 18,5 heures (12h30-18h00).
...
Si on met tout ensemble, le script est le suivant :
...
Je l'utilise depuis 2 ans et cela fait une grande différence dans la façon dont la température est ressentie. C'est un game changer pour moi.

### **Différents déclencheurs pour réduire le nombre d'exécutions**

Comme je l'ai dit précédemment, nous pouvons utiliser un déclencheur différent au lieu de `time_pattern` pour réduire les exécutions. Même sans exécuter l'action, la vérification des conditions utilise un peu de CPU.

En regardant notre automatisation, le véritable déclencheur est la température : l'extérieur est supérieur de plus de 2 degrés à l'intérieur et l'intérieur est supérieur à 20 degrés.  
...
Cela se déclenche en fonction de la condition extérieur vs intérieur, et nous vérifions si la valeur reste vraie pendant au moins 5 minutes.  
Nous pourrions ajouter un deuxième déclencheur pour la température intérieure, mais les déclencheurs utilisent une logique **OR** : si l'un est vrai, l'action s'exécute (bien que les conditions puissent l'empêcher).
...
C'est à vous de décider du meilleur déclencheur et du nombre d'exécutions incorrectes que vous êtes prêt à accepter.

### Amélioration future

C'est la version que j'ai utilisée jusqu'à présent, mais elle s'est parfois déclenchée de manière incorrecte. Comme elle est basée sur la température, les conditions estivales peuvent être remplies même lorsqu'il pleut dehors. Dans ces conditions, la température intérieure ne monte pas car le soleil ne passe pas par les fenêtres.  
À la fin de l'été, j'ai installé des détecteurs de mouvement extérieurs pour ajouter un nouveau paramètre : *l'intensité lumineuse*.
