...
- Open Source
- Home Assistant
description: 'Comment auto-héberger l''émulateur de station de base Arlo arlo-cam-api : stack Docker avec RTSP à la demande via MediaMTX, sidecar Flask arlo-snapshot personnalisé, et les trois correctifs que j''ai contribué en amont pour corriger un bug de démarrage Flask, ajouter un beacon de maintien en vie, et activer snapshot-on-motion.'
cover: cover.jpg
showHero: true
---

Ceci est l'article 2 d'une série de trois sur le remplacement de la station de base Arlo propriétaire par une stack auto-hébergée. Dans l'[article 1 de cette série](/replacing-arlo-base-station-with-a-netgear-orbi-router/) j'ai couvert la couche réseau — comment faire en sorte qu'un Netgear Orbi RBR760 se fasse passer pour la station de base Arlo suffisamment bien pour que les caméras se connectent, s'enregistrent et streament. Dans cet article, je couvre la couche *serveur* : la stack Docker qui tourne réellement sur le mini PC, le sidecar Flask personnalisé qui capture des snapshots déclenchés par le mouvement, le relais RTSP à la demande qui rend le streaming en direct économe en batterie, et les trois pull requests upstream que j'ai contribué pour corriger les bugs que j'ai rencontrés en chemin.

Le troisième et dernier article couvrira Home Assistant — capteurs REST, cartes picture-glance, armement/désarmement, webhooks de mouvement — une fois que le reste de la stack sera solide. Le dépôt compagnon sur [github.com/mmornati/arlo-base-station](https://github.com/mmornati/arlo-base-station) contient tous les fichiers de configuration mentionnés ici.

> **Une note sur la rédaction.** Tout au long de cet article, les vrais numéros de série des caméras, les adresses MAC, et l'IP LAN de production du serveur ont été remplacés par des placeholders de style `XXXXXXXXXXXX` et `192.168.1.X`. La valeur bien connue `172.14.1.1` de la passerelle Arlo est conservée car elle fait partie du protocole wire. Le sous-réseau invité `192.168.2.x` (où vivent les caméras sur l'Orbi) est laissé tel quel car c'est la valeur par défaut standard d'Orbi et ne révèle rien de spécifique.
...
- **Pas d'abonnement.** Les caméras sont des VMC4040P (Arlo Pro 4), ce qui signifie qu'elles continuent à fonctionner sans abonnement — mais les alertes de mouvement sont limitées aux push uniquement, vous perdez la timeline cloud, et le jour où Arlo décidera de retirer le niveau gratuit, les caméras deviendront de coûteux presse-papiers. Le mode local-only supprime ce risque.
...
Notez les trois lignes de bind-mount sur `arlo-cam-api` qui chargent `./server.py`, `./api/api.py`, et `./config.yaml` par-dessus les copies de l'image upstream. C'est le workaround de correctif de production qui me permet de livrer les fixes des PRs #29, #30, et #31 sans forker l'image. J'y reviendrai dans la section « Correctifs de production » plus bas.
...
Out of the box, `arlo-cam-api` expose `GET /snapshot/<serial>`, mais avec un caveat fatal : l'endpoint ne retourne 200 que si un snapshot a été *poussé* dessus par la caméra — et les caméras VMC4040P **ne poussent jamais**. Appeler le `snapshot_request()` URL de la caméra via le protocole de la station de base retourne un ACK de la caméra, puis ferme la connexion proprement sans uploader un seul octet. Donc `SnapshotCount` reste à 0 pour toujours et Home Assistant affiche une carte caméra vide.
...
```
Déclencheur PIR
  -> la caméra envoie pirMotionAlert sur TCP/4000
    -> le handler server.py se déclenche
      -> POST http://arlo-snapshot:8000/snapshot/<serial>
        -> arlo-snapshot appelle POST /device/<serial>/userstreamactive {active:true}
        -> dort STREAM_WARMUP_SEC (6s) pour que le serveur RTSP s'ouvre
        -> ouvre rtsp://<cam-ip>:555 ou 554 avec PyAV, décode une frame
        -> redimensionne à MAX_WIDTH=1280, encode JPEG quality 75
        -> stocke dans un dict en mémoire indexé par serial
HA still_image_url GET
  -> http://<server>:8000/snapshot/<serial>
    -> retourne le JPEG stocké (pas de RTSP, pas d'interaction caméra)
```
...
- **`maxReaders: 5`** — Home Assistant ouvre plusieurs connexions RTSP par caméra (preview + flux principal + détecteur Frigate si vous l'utilisez). La valeur par défaut de MediaMTX qui est 2 est trop basse et vous obtenez des erreurs `maximum reader count reached`. Cinq est confortable pour HA + un ou deux viewers externes.
...
C'est la pièce maîtresse de l'article. Après avoir déployé `arlo-cam-api` contre les vraies caméras, trois bugs distincts sont apparus en moins d'une semaine. Les trois sont maintenant des pull requests upstream contre [brianschrameck/arlo-cam-api](https://github.com/brianschrameck/arlo-cam-api) :
...
Un nouveau `BeaconThread` qui, toutes les `BeaconIntervalSeconds` (par défaut 60s), envoie une `statusRequest` légère à chaque caméra vue via le trafic entrant. La `statusRequest` retourne un ACK + une réponse de statut, ce qui double comme sonde de liveness.
...
L'intervalle de 60 secondes est conservateur. Le seuil pour `MaxMissedBeaconTime` est 30, mais la cadence exacte à laquelle Arlo compte les beacons ratés n'est pas documentée. 60s vous donne une marge de sécurité de 30x qui semblait confortable ; vous pourriez descendre aussi bas que 10s si vous le vouliez, au prix d'une charge légèrement plus élevée sur la station de base.
...
Le défaut est `false` pour que les déploiements sans `arlo-snapshot` qui tourne ne soient pas affectés. Sur mon déploiement de production c'est `true`, et les déclencheurs PIR résultent en un JPEG `<serial>` frais qui arrive dans le store en mémoire d'`arlo-snapshot` en ~10 secondes.
...
Le workaround c'est le bind-mount. Le conteneur `arlo-cam-api` expose `/opt/arlo-cam-api/` avec les fichiers source. Trois lignes dans `docker-compose.yaml` superposent mes copies patchées par-dessus l'image upstream :
...
server/
  ...
  config.yaml         # patché (BeaconIntervalSeconds, SnapshotOnMotion, suppression de webhook)
  mediamtx.yml
  arlo-snapshot/
    Dockerfile
    app.py
...
- **Pas de support POST `device.snapshot_request()` sur VMC4040P.** C'est une limitation firmware — la caméra accepte la commande et ACK mais n'uploade jamais. PR #31 le contourne via le grab du sidecar. Une PR future pourrait exposer un endpoint `/camera/<serial>/snapshot` sur `arlo-cam-api` qui wrap le sidecar et retourne le JPEG de façon synchrone, simplifiant l'intégration.

Aucun de ceux-ci n'est bloquant. Ce sont des nice-to-haves sur lesquels je reviendrai quand j'y reviendrai.

## Et ensuite

Cet article a couvert la couche serveur : la stack Docker, le sidecar snapshot personnalisé, le relais RTSP à la demande, et les trois correctifs upstream. Le prochain — et dernier — article de la série couvre le côté Home Assistant : capteurs REST pour batterie et signal, cartes picture-glance pour le dashboard, contrôles d'armement/désarmement, webhooks de mouvement, et le layout Lovelace. Ce sera dans l'[article 3 de cette série](/integrating-self-hosted-arlo-with-home-assistant/) (le lien ne se résoudra pas tant que la phase 4 ne sera pas publiée).

Pour le côté réseau, voir l'[article 1 de cette série](/replacing-arlo-base-station-with-a-netgear-orbi-router/).
