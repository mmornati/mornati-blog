...
Il y a trois ans, j'ai emménagé dans une maison où le précédent propriétaire avait laissé une station de base Arlo unique dans le bureau à l'étage et trois caméras Pro 4 réparties dans le jardin. L'application Arlo fonctionnait, le cloud était gratuit pendant la période d'essai, et le système m'envoyait fidèlement des snapshots quand le facteur passait devant le portail. Puis l'essai a expiré, la batterie de *Caméra Jardin 2* est morte en quatre jours, et la caméra dans le coin du jardin refusait de se connecter plus d'une fois sur deux — son signal WiFi venait de la station du bureau à travers deux murs en briques et un volet métallique. Ça vous rappelle quelque chose ?
...
Si vous avez une seule station de base Arlo dans un coin de votre maison, les caméras à l'autre bout reçoivent un signal médiocre et perdent la connexion. Votre mesh Orbi (routeur + 2 satellites) couvre toute la maison magnifiquement, mais les caméras ne peuvent pas l'utiliser — elles ne voient jamais que le SSID Arlo, et elles ne parleront jamais qu'à la boîte qui l'abroadcast.
...
Vérifiez avec `iptables -L FORWARD -n -v --line-numbers` — les règles utilisateur doivent être aux lignes 1-3, et `ODM_FORWARD_TOP` doit apparaître à la ligne 4 ou plus tard. Si l'ordre est inversé, la caméra ne s'enregistrera pas.
...
Le script de démarrage complet, idempotent, qui gère les bizarreries Netgear est à [`rbr760/S99arlo`](https://github.com/mmornati/arlo-base-station/blob/main/rbr760/S99arlo) dans le dépôt compagnon. Il fait 60 lignes, commentaires inclus. Les trois bits non évidents sont extraits ci-dessous.
...
Le Post 2 complet couvre : `server.py` patché pour corriger le bug auto-register-on-restart, le handler de mouvement `arlo-snapshot` à deux endpoints, et la layout Docker Compose. Pour cet article l'important est que :
...
Si `device` retourne `[]` après 5 minutes, la cause la plus probable est le bug SNAT-from-camera-direction de §4.4. Vérifiez `conntrack` pour `SYN_RECV` et démolissez.
...
Si les règles manquent après un reboot mais que `S99arlo` est dans `/etc/rc.d/`, alors il est en course avec l'init Netgear. Augmentez le numéro de démarrage (par ex. `S99arlo` → `S991arlo`).
...
Cet article a couvert la partie qui a la pire courbe « si vous ne savez pas, vous ne pouvez pas la Googler » : faire en sorte que le routeur ressemble à une station de base. Les couches restantes sont plus conventionnelles :
